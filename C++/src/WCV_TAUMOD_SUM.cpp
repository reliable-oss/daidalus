...
WCV_TAUMOD_SUM::WCV_TAUMOD_SUM() {
  if (wcv_vertical != NULL) {
    // wcv_vertical is allocated on construction of WCV_TAUMOD,
    // which this class is derived from. We need to delete before
    // allocating again to avoid leaking memory.
    delete wcv_vertical;
  }
